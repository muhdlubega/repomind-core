...
export async function getAccessibleRepository(db: D1Database, repositoryId: string, principal: Principal): Promise<RepositoryRow> {
  const row = await db.prepare("SELECT * FROM repositories WHERE id = ? AND is_demo = 0 AND (owner_id IS NULL OR owner_id = ?)").bind(repositoryId, principal.userId).first<RepositoryRow>();
