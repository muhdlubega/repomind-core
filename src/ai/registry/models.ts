...
import { LangChainGeminiProvider } from "../providers/langchain-gemini";
import { createMistralProvider } from "../providers/openai-compatible";
...
export class ModelRegistry {
  ...
  definition(purpose: ModelPurpose, requested?: ProviderId): ModelDefinition {
    ...
    if (provider === "gemini") return { provider, model: this.config.GEMINI_MODEL, purpose };
  ...
  provider(purpose: ModelPurpose, requested?: ProviderId): CodeLensaModelProvider {
    ...
    if (definition.provider === "gemini" && this.config.GEMINI_API_KEY && definition.model) return new LangChainGeminiProvider(this.config.GEMINI_API_KEY, definition.model);
